...
def add_movie():
    ...
    try:
        movie = db.session.get(Movie, movie_id_int)
        if movie:
            ...
            movie = Movie(
                ...
                year=int(movie_info["year"]) if movie_info["year"] and str(movie_info["year"]).isdigit() else None,
        ...
        db.session.flush()
        # sync genres: add new, remove stale
        incoming = set(movie_info.get("genres", []))
        existing = {g.genre for g in movie.genres}
        for g in incoming - existing:
            db.session.add(Genre(tmdb_id=movie_id_int, genre=g))
        for g in list(movie.genres):
            if g.genre not in incoming:
                db.session.delete(g)
        # remove from watchlist
        wl = db.session.get(Watchlist, movie_id_int)
        if wl:
...
def add_movie_watchlist():
    ...
    try:
        if db.session.get(Watchlist, movie_id_int):
...
def route_search_movie():
    ...
    if not search_results:
        return jsonify({"error": "Aucun résultat trouvé pour ce titre."}), 400
    return jsonify({"results": search_results})


@bp.route("/update_movie/<int:tmdb_id>", methods=["POST"])
...
def delete_movie_watchlist(tmdb_id):
    try:
        wl = db.session.get(Watchlist, tmdb_id)
        if wl:
            db.session.delete(wl)
            db.session.commit()
        return "", 204
